...
import com.ozeeesoftware.employeemanagement.exception.FileNotFoundException;
...
public class ImageModelServiceImpl implements ImageModelService{
    ...
    @Autowired
    public ImageModelServiceImpl(ImageStorageProperties imageStorageProperties) {
    ...
    @Override
    public ResponseEntity<Object> addProfileImage(MultipartFile image, long employeeId){
        String randomFileName= UUID.randomUUID().toString();
        String extension="."+ FilenameUtils.getExtension(image.getOriginalFilename());
        String newFileName=randomFileName+extension;
        String finalFileName= StringUtils.cleanPath(newFileName);
        ...
        try {
            ...
            if (extension.matches(".png|.jpeg|.jpg")) {
                ...
                tmpFile.setName(finalFileName);
                tmpFile.setUrl(fileDownloadUri);
                Optional<Employee> existingUser=employeeRepository.findById(employeeId);
